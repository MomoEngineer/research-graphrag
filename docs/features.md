...
| **Personenkennung der Autoren** | Speichert je Autor die OpenAlex-Autor-ID und die ORCID, geprüft und positionsgleich zu den Namen; Referenz-Einträge bringen ihre Autoren aus der Stub-Datei mit; jeder Ingest weist die Autoren-Abdeckung der Volltexte aus | in `ingest` (Tabelle `paper_metadata`), `author_identities` in `get_paper`/`get_reference`/`answer_question` | [bibliography/model](../src/research_graphrag/bibliography/doc/model.md), [indexing/metadata_index](../src/research_graphrag/indexing/doc/metadata_index.md), [extraction/refstub](../src/research_graphrag/extraction/doc/refstub.md) | [0041](adr/0041-author-identity-and-schema.md) |
