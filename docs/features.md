...
| **Seite-1-Beleg und Ablehnungsvermerk** | Prüft schwach belegte Zitierdaten gegen die Titelseite des PDFs: bestätigt ⇒ `strong`, fremdes Paper ⇒ verworfen und dauerhaft gesperrt; ein zweiter Auflösungslauf spielt keinen verworfenen Treffer zurück. Scharfgeschaltet erst nach Kalibrierung (A0) | `python -m scripts.verify_metadata`, in `resolve_metadata` | [bibliography/titlepage](../src/research_graphrag/bibliography/doc/titlepage.md), [bibliography/triage](../src/research_graphrag/bibliography/doc/triage.md) | [0042](adr/0042-title-page-evidence-and-rejections.md) |
| **LLM-Arbeitsliste** | Das LLM schlägt je Paper nur Kennung oder Titel vor, das Skript löst auf und übernimmt nur Bestätigtes; „nicht auflösbar“ wird mit Grund ausgewiesen (`review` in den Werkzeugen) | `python -m scripts.metadata_worklist` | [bibliography/worklist](../src/research_graphrag/bibliography/doc/worklist.md) | [0042](adr/0042-title-page-evidence-and-rejections.md) |
...
| **Hybrid-Wertung** | BM25 und TF-IDF-Kosinus über **einer** Tokenisierung, verbunden per Reciprocal Rank Fusion; umschaltbar auf ein Einzelverfahren; seit Phase 16 bit-identisch ohne volle Matrixmultiplikation (spaltenweise über die Anfrage-Terme, gemeinsame Wertung je Anfrage) | `--scoring` bzw. Parameter `scoring` | [indexing/bm25](../src/research_graphrag/indexing/doc/bm25.md), [indexing/fusion](../src/research_graphrag/indexing/doc/fusion.md), [indexing/tfidf_index](../src/research_graphrag/indexing/doc/tfidf_index.md) | [0014](adr/0014-hybrid-retrieval-bm25-tfidf-phase7.md), [0044](adr/0044-response-latency-bit-identical-scoring-and-fts5-phase16.md) |
...
| **Personenkennung der Autoren** | Speichert je Autor die OpenAlex-Autor-ID und die ORCID, geprüft und positionsgleich zu den Namen; Referenz-Einträge bringen ihre Autoren aus der Stub-Datei mit; Kennungen bereits aufgelöster Paper werden ohne Netz aus den Rohantworten nachgetragen; jeder Ingest weist die Autoren-Abdeckung der Volltexte aus | in `ingest` (Tabelle `paper_metadata`), `python -m scripts.backfill_author_ids`, `author_identities` in `get_paper`/`get_reference`/`answer_question` | [bibliography/model](../src/research_graphrag/bibliography/doc/model.md), [indexing/metadata_index](../src/research_graphrag/indexing/doc/metadata_index.md), [extraction/refstub](../src/research_graphrag/extraction/doc/refstub.md), [online/backfill](../src/research_graphrag/online/doc/backfill.md) | [0041](adr/0041-author-identity-and-schema.md) |
| **Autorenindex (Personenebene)** | Leitet beim Index-Bau je Paper und Autorposition eine Zeile ab – nur aus `strong` belegten Zitierdaten, mit Personenschlüssel (Kennung vor Name) und Trigramm-Namenssuche; kein Retrieval-Modus liest sie, nur die Personen-Werkzeuge | in `ingest`, Tabellen `paper_authors`/`author_name_search` | [indexing/author_index](../src/research_graphrag/indexing/doc/author_index.md), [indexing/fts](../src/research_graphrag/indexing/doc/fts.md) | [0043](adr/0043-author-index-and-person-tools.md) |
| **Phrasenindex über die Chunks** | FTS5-Volltextindex (`unicode61`, ohne Diakritika) über den Chunk-Text: Wortfolge, Wortanfang und Nähe zweier Begriffe, mit Provenienz, Ausschnitt und Gesamtzahl; **Infrastruktur** für Phase 17 / A5, greift in kein Ranking ein | in `ingest`, Tabelle `chunk_fts`; intern `indexing.chunk_fts.search_phrase`/`search_prefix`/`search_near` | [indexing/chunk_fts](../src/research_graphrag/indexing/doc/chunk_fts.md), [indexing/fts](../src/research_graphrag/indexing/doc/fts.md) | [0044](adr/0044-response-latency-bit-identical-scoring-and-fts5-phase16.md) |
...
| **Original-PDF lokal auffinden** | Liefert den nativen Dateisystem-Pfad des Original-PDFs (kein Dateiinhalt); Referenz-Einträge und lokal fehlende PDFs sind sichtbar `available = false`, kein Fehler | `get_paper_file` | [retrieval/paper_file](../src/research_graphrag/retrieval/doc/paper_file.md) | [0039](adr/0039-correction-tool-and-pdf-file-access.md) |
| **Personen-Werkzeuge** | Autoren als Rechercheebene: Kandidaten zu einem Namen (Mehrdeutigkeit sichtbar, nie still zusammengeführt), Profil mit Papern, Communities und direkten Mitautoren, Basic-Suche beschränkt auf die Paper einer Person, Zitationsnetz der Person im Korpus mit markierten Selbstzitaten; jede Antwort weist die Abdeckung der Personenebene aus | `search_authors`, `get_author`, `search_author_papers`, `get_author_citations`, `python -m scripts.authors` | [retrieval/authors](../src/research_graphrag/retrieval/doc/authors.md) | [0043](adr/0043-author-index-and-person-tools.md) |
| **Metadaten-Korrektur** | Erstes schreibendes Werkzeug: korrigiert Titel/Autoren/Jahr/Venue/DOI/arXiv/URL als `manual`-Herkunft (feldweiser Merge, Pflichtbeleg, append-only Protokoll); wirksam erst beim nächsten Ingest | `correct_paper_metadata` | [bibliography/corrections](../src/research_graphrag/bibliography/doc/corrections.md) | [0039](adr/0039-correction-tool-and-pdf-file-access.md) |
...
| Feature | Was es leistet | Einstiegspunkt | Verbaut in | Grundlage |
| --- | --- | --- | --- | --- |
| **MCP-Server (stdio)** | Stellt fünfzehn Werkzeuge für GitHub Copilot bereit und hält den Index in einem Prozess-Cache, der über den Dateizustand verfällt – neue Paper wirken ohne Neustart; seit Phase 16 / F3 lädt er den Index beim Start im Hintergrund vor | `python -m research_graphrag.mcp_server` | [mcp_server/server](../src/research_graphrag/mcp_server/doc/server.md) | [0009](adr/0009-mcp-server-stdio-phase5.md), [0010](adr/0010-drop-in-workflow-and-qa-phase6.md) |
...
| **Kommandozeile** | Skripte für Intake, Ingestion, Fragen, Zitation, Personen, Status, Sicherung, QS, Online-Recherche, Metadatenpflege und Evaluation – der vollständige Funktionsumfang ohne Copilot | `python -m scripts.<name>` | [scripts/README.md](../scripts/README.md) | — |

### Die fünfzehn MCP-Werkzeuge
...
| `get_paper_file` | Lokaler Dateipfad des Original-PDFs (kein Dateiinhalt) | [get_paper_file.md](../src/research_graphrag/mcp_server/specs/get_paper_file.md) |
...
| `correct_paper_metadata` | Bibliografische Korrektur (manual-Herkunft), wirksam erst beim nächsten Ingest | [correct_paper_metadata.md](../src/research_graphrag/mcp_server/specs/correct_paper_metadata.md) |
| `search_authors` | Personen-Kandidaten zu einem Namen, samt Identitätsstatus und Mehrdeutigkeit | [search_authors.md](../src/research_graphrag/mcp_server/specs/search_authors.md) |
| `get_author` | Profil einer Person: Paper, Jahresspanne, Communities, direkte Mitautoren | [get_author.md](../src/research_graphrag/mcp_server/specs/get_author.md) |
| `search_author_papers` | Basic-Suche nur in den Papern einer Person | [search_author_papers.md](../src/research_graphrag/mcp_server/specs/search_author_papers.md) |
| `get_author_citations` | Wer zitiert die Person, wen zitiert sie – innerhalb des Korpus | [get_author_citations.md](../src/research_graphrag/mcp_server/specs/get_author_citations.md) |
...
| **Handprobe Tool-Wahl (Personen)** | Feste Vorlage mit zehn Personenfragen und fünf Kontrollfragen bestehender Fragetypen für den realen Client; Schwelle 9 von 10 richtig gewählte Personen-Werkzeuge ohne Verschlechterung der Kontrollfragen – verfehlt heißt: Beschreibungen schärfen, nicht Werkzeuge streichen | [eval/personen-toolwahl.md](../eval/personen-toolwahl.md) | [mcp_server/server](../src/research_graphrag/mcp_server/doc/server.md) | [0043](adr/0043-author-index-and-person-tools.md) |
...
| **Sicherung des Bestandes** | Sichert idempotent nur das nicht Reproduzierbare (PDFs, Übersicht, kuratiertes Relevanzurteil, Zitationsdaten samt Ablehnungsvermerken, abgelegte LLM-Antworten der Metadaten-Arbeitsliste, Manifest, Protokolle) in ein Verzeichnis außerhalb der Arbeitskopie und rechnet den Stand auf Wunsch gegen sein Prüf-Manifest nach | `python -m scripts.backup --ziel <Pfad>` | [backup](../src/research_graphrag/doc/backup.md) | [0027](adr/0027-corpus-backup-phase11.md), [0034](adr/0034-decommission-uebersicht-and-inflow-stop-rule-phase15.md) |
