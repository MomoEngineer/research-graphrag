...
| --- | --- | --- |
| A0 | Festlegungen fixiert. **Messpaket übergeben** (Wegwerf, außerhalb des Repos, nur lesend – am Mini-Korpus per Prüfsumme belegt): Abdeckung inkl. Stub-Lücke, Nachtrag und Laufbedarf, Identität, Kalibrierliste samt Auswertung, Sammelabfrage-Probe, Handproben-Vorlage und seit A4 die Auswertung der Handprobe gegen `search_authors`/`get_author` (Recall, Fehltreffer, nur über Namensidentität erreichbare Paper). **Messung am realen Bestand steht aus** | dieser Block |
...
| A2, Punkte 3–5 | **Punkte 3 und 4 umgesetzt:** Nachtrag der Kennungen aus `data/online_raw/` ohne Netz (`scripts.backfill_author_ids`, nur bei identischer Namensliste, Konflikte ausgewiesen); Auflösungslauf fortsetzbar (Zwischenstand alle zehn Paper und bei Abbruch, Halt bei HTTP 429). **Offen bis A0:** Sammelabfragen (erst messen) und Punkt 5, `MAX_AUTHORS`; die Kürzung auf 25 bleibt bis zur Messung | [ADR 0041, Nachtrag](docs/adr/0041-author-identity-and-schema.md#nachtrag-2026-09-24-a2-punkte-3-bis-5) |
...
- **Hybrid-Suche mit dedizierten Vektor-/Suchmaschinen** und **Skalierung Richtung Qdrant/Weaviate/Neo4j**. *Dieser Punkt ist seit [Phase 15](docs/roadmap-historie.md#phase-15--skalierung-den-wachsenden-bestand-tragen) nicht mehr die Antwort auf Wachstum:* Die dort bezifferten Wände sind Implementierungsdetails im eigenen Code, keine Grenzen des Speichermodells – ein fremdes Backend bliebe für einen vierstelligen Bestand überzogen und ist offline ohnehin nicht beschaffbar. Der Bestand liegt inzwischen (3.461 Paper) jenseits der in [G5](docs/roadmap-historie.md#g5--auslegung-neu-festschreiben) festgeschriebenen Auslegung – die Einschätzung bleibt trotzdem unverändert: [ADR 0038](docs/adr/0038-corpus-ceiling-revision-local-search-latency.md) empfiehlt als nächsten Schritt **FTS5** (eingebettet in SQLite, offline beschaffbar), keinen Sprung auf ein externes Backend.
